...
if [ "$HELP" -eq 1 ]; then
    ...
    echo ""
    echo "This script orchestrates a static V2X performance experiment using the lightweight producer."
    echo "It is optimized for high-density client simulation with lower resource overhead."
    echo ""
    echo "Options:"
...
for CAR_NUM in "${NUM_CAR[@]}"; do
    ...
    $COMMON_SCRIPT_ROOT/script/check_consumer_group.sh --config $COMMON_SCRIPT_ROOT/config/common.config --broker $KAFKA_BROKER --prefix "r_group_" --count $SERVICE_CNT $VERBOSE_TAG
    echo "--------------------------------------------------"
