...
if [ "$HELP" -eq 1 ]; then
    ...
    echo ""
    echo "This script orchestrates a dynamic V2X performance experiment using the lightweight producer."
    echo "It is optimized for simulating time-varying traffic load with high-density clients."
    echo ""
    echo "Options:"
...
MEASURE_CONSUMER_ID="MeasuerConsumer_Dynamic"
...
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
echo "[7/9] Terminating Connector and measuring Consumer ($TIMESTAMP)"
clean_up_connector $CONNECTOR_ID $TERMINATE_TIMEOUT
clean_up_measure_consumer $MEASURE_CONSUMER_ID $TERMINATE_TIMEOUT
echo "--------------------------------------------------"
